...
export function ProductShowcase({ onAddToCart, searchQuery }: ProductShowcaseProps) {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    switch (sortBy) {
      case "price-low":
        filtered.sort((a: Product, b: Product) => Number.parseFloat(a.price) - Number.parseFloat(b.price))
        break
      case "price-high":
        filtered.sort((a: Product, b: Product) => Number.parseFloat(b.price) - Number.parseFloat(a.price))
        break
      case "newest":
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {filteredAndSortedProducts.map((product: Product) => (
            <div
              key={product.id}
              className="glass-card rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 product-card"
            >
              <div className="relative">
                ...
                >
                  <Heart
                    className={`w-4 h-4 ${wishlist.has(product.id) ? "text-red-500 fill-current" : "text-gray-400"}`}
                  />
                </Button>
              </div>
                ...
                <div className="flex items-center mb-4">
                  <div className="flex text-yellow-400">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        key={i}
                        className={`w-4 h-4 ${i < Math.floor(Number.parseFloat(product.rating || "0")) ? "fill-current" : ""}`}
                      />
                    ))}
                  </div>
